...
class Connection
{
    ...
    protected function addClauseCondWithBind($clause, $andor, $conditions)
    {
        ...
        foreach ($conditions as $k => $value) {
            if (is_array($value)) {
                ...
                $bind[] = implode("", $value[1]);

                $clause =& $this->$clause;
                $cond = $cond ." "."$ab[0]"." " .'('.implode(',', $ab[1]).')';
            } else {
                $bind[] = $value;
                $clause =& $this->$clause;
                /*新增*/
     ...
     * @return self
     */
    public function data(array $cols)
    {
     ...
     * @return self
     */
    protected function addCol($col , $val)
    {
        $key                    = $this->quoteName($col);
        $this->col_values[$key] = "$val";
        $args                   = func_get_args();
        if (count($args) > 1) {
    ...
    protected function execute($query, $parameters = "")
    {
        try {
            ...
            if ($e->errorInfo[1] == 2006 || $e->errorInfo[1] == 2013) {
                ...
                $this->rollBackTrans();
                $msg = $e->getMessage();
                $err_msg = "SQL:".$this->getQuerySql()." ".$msg;
     ...
     * @return mixed
     */
    public function all($keyy = '',$query = '', $params = null, $fetchmode = PDO::FETCH_ASSOC)
    {
        $query = trim($query);
        if (empty($query)) {
            $query = $this->build();
        ...
        if ($statement === 'select' || $statement === 'show') {
            $arr = $this->sQuery->fetchAll($fetchmode);
            if (!empty($keyy)) {
                $arrr = array();
                foreach ($arr as $key => $value) {
                     foreach ($value as $k => $v) {
                         $arrr[$value['uid']] = $value;
                     }
                }
            }
            return $arrr;
     ...
     * @return array
     */
    public function one($query = '', $params = null, $fetchmode = PDO::FETCH_ASSOC)
    {
        $query = trim($query);
        if (empty($query)) {
            $query = $this->build();
     ...
     * @return  string
     */
    public function getQuertSql()
    {
        return $this->getQuertSql;
    }
    ...
    public function increment(array $cols)
    {

        return $this->crement($cols);
    }
     ...
     * 返回所影响的行数
     */
    public function decrement(array $cols)
    {
        return $this->crement($cols, '-');
    }


    private function crement(array $cols, $icon = '+')
    {
        foreach ($cols as $k => $v) {
            $k = $k;
            $v = $v;
        }
        $key                    = $this->quoteName($k);
        $this->col_values[$key] = "$k$icon$v";
        return $this;
    }
