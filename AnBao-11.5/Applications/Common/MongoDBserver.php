...
class MongoDBserver {
     ...
     * )
     **/
    public static function init($colName){

        $host = Config::$MongoDBConfig['host'];
        $port = Config::$MongoDBConfig['port'];
        if (!self::$_mongo) {
            try {
                self::$_mongo = new \MongoClient("mongodb://$host:$port", array('connect'=>true));// 立即连接
        ...
        self::$databasename = $colName;
//        //重启删除数据库
//        self::dropDB($colName);

        //选择的数据库
        //没有新增。
        self::selectDB($colName);
        echo "MongoDB Server is ready start \n";
//    echo "connect\n";
...
//    }
     ...
     * @return boolean
     */
    public static function insert($colName, $sets, $safe=false, $fsync=false){
     ...
     * @return boolean
     */
    public static function save($colName, $sets, $safe=false, $fsync=false){
        // 处理 '_id' 字段
        $sets = self::_parseId($sets);
        $ret = self::_getCol($colName)->save($sets,array('w'=>$safe,'fsync'=>$fsync));
        return $ret;
    }
    /**
     * 删除集合中的文档记录
     ...
     * @return boolean
     */
    public static function delete($colName,$query=array(),$delAll=true,$safe=false,$fsync=false){
        // 自动处理 '_id' 字段
        $query = self::_parseId($query);
        // 删除选项
        $options = array(
            'justOne'  =>  !$delAll,
            'w'      =>  $safe,
     ...
     * @return array
     */
    public static function dropCol($colName){
        return self::_getCol($colName)->drop();
    }
    /**
     * 更新集合文档记录
     *
     ...
     * @return boolean
     */
    public static  function update($colName,$newDoc,$query=array(),$option='set',$upAll=true,$upsert=false,$safe=false,$fsync=false){
        // 自动处理 '_id' 字段
        $query = self::_parseId($query);
        // 得到集合
        $col = self::_getCol($colName);
        // 重新组合新文档
        if($option != 'replace'){
            $newDoc = array(self::cmd($option) => $newDoc);
        }
        // 更新条件
        $options = array(
            'upsert'  =>  $upsert,
     ...
     * @return array
     */
    public static function select($colName,$query=array(),$fields=array(),$sort=array(),$limit=0,$skip=0){
        // 得到集合
        $col = self::_getCol($colName);
        // 自动处理 '_id' 字段
        $query = self::_parseId($query);
     ...
     * @return unknown
     */
    public static function count($colName,$query=array(),$limit=0,$skip=0){
        return self::_getCol($colName)->count($query,$limit,$skip);
    }
    /**
     * 返回集合中的一条记录(一维数组)
     *
     ...
     * @return array
     */
    public static function fetchRow($colName,$query=array(), $fields=array()){
        // 得到集合名
        $col = self::_getCol($colName);
        // 自动处理 '_id' 字段
        $query = self::_parseId($query);
        // 处理结果集
        return self::_parseArr($col->findOne($query,$fields));
    }
    /**
     * 返回符合条件的文档中字段的值
     *
     ...
     * @return mixed
     */
    public static function fetchOne($colName,$query=array(), $fields='_id'){
        $ret = self::fetchRow($colName,$query,array($fields));
     ...
     * @return array
     */
    public static function fetchCol($colName,$query=array(), $fields='_id'){
        $result = array();
        $list = self::select($colName,$query,array($fields));
        foreach ($list as $row){
            $result[] = $row[$fields];
        }
        return $result;
     ...
     * @return array
     */
    public static function fetchAssoc($colName,$query=array(), $fields='_id'){
        $result = array();
        $list = self::select($colName,$query);
        foreach ($list as $row){
            $key = $row[$fields];
            $result[][$key] = $row;
        }
     ...
     * @return string
     */
    public static function cmd($option=''){
        // 只返回命令前缀
        if($option == ''){
            return self::$_cmd;
        }
        // 如果是操作符
        if(isset(self::$_condMap[$option])){
            $option = self::$_condMap[$option];
        }
        return self::$_cmd.$option;
    }
    /**
     ...
     * @return array
     */
    public static function allDB($onlyName=false){
        $ary = self::$_mongo->listDBs();
        if($onlyName){
            $ret = array();
    ...
    public static function dropDB($dbname){
        return self::$_mongo->dropDB($dbname);
    }
    /**
     * 关闭连接
     *
    ...
    private static function _parseId($arr){
        if(isset($arr['_id'])){
    ...
    // 得到集合对象
    private static function _getCol($colName){
        return self::$_db->selectCollection($colName);
    }
    // 解析数组中的"_id"并且返回
